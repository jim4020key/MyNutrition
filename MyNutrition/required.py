...
def nutrition(age, weight, height, gender, activity_level):
    ...
    nutrition_list = [aaEER, Macronutrients['Protein,g'],Macronutrients['TotalFat,kcal'], Macronutrients['Carbonhydrate,g'],
                      Macronutrients['DietaryFiber,g'],Minerals['Calcium,mg'],Minerals['Iron,mg'], Minerals['Magnesium,mg'],
                      Minerals['Phosphorus,mg'], Minerals['Potassium,mg'], Minerals['Sodium,mg'], Minerals['Zinc,mg'],
                      Minerals['Manganese,mg'], Minerals['Selenium,mcg'], Vitamins['Vitamin B6, mg'],  Vitamins['Vitamin E,mg AT'],
                      Vitamins['Vitamin K, mcg'], Vitamins['Vitamin B12, mcg'], Vitamins['Riboflavin, mg'], Vitamins['Niacin, mg'],
                      Vitamins['Vitamin C, mg'], Vitamins['Folate, mcg DFE'], Vitamins['Vitamin A, mg RAE']] 
                        # Vitamins['Choline, mg'],
...
"""형식에 맞게 정렬"""
def required_dict(nutrition_list):
    namelist = ['Energy','Protein','Fat','Carbohydrate','Dietary_Fiber','Calcium','Iron','Magnesium','Phosphorus','Potassium','Sodium','Zinc','Manganese','Selenium','Vitamin_B6','Vitamin_E','Vitamin_K','Vitamin_B12','Riboflavin','Niacin','Total_Vitamin_C','Folate','Vitamin_A']
    required_dict = {}
    startindex = 0
    for n in namelist:
        required_dict[n] = nutrition_list[startindex]
        startindex +=1
    return required_dict

def short_or_extra(my, required):
    keys = my.keys()
    inshort = {}
    extra = {}
    total = {}
    for key in keys:
        if my[key] < required[key]:
            inshort[key] = required[key] - my[key]
            total[key] = required[key] - my[key]
        elif my[key] > required[key]:
            extra[key] = my[key] - required[key]
            total[key] = required[key] - my[key]
    return inshort, extra, total

        

    

#print(nutrition(25, 52, 1.6, 'female', 'Active'))
#nutrition_list = nutrition(25, 52, 1.6, 'female', 'Active')
#print(required_dict(nutrition_list))
